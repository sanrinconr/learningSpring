...
public class PatientServiceTest {
    ...
    public void getPatient() throws PatientNotExistsException {
        when(patientRepository.findById(any(Long.class))).thenReturn(Optional.of(PatientModel.builder().name("A").last_name("A").build()));
        PatientResponseDTO patient = patientService.getById(100L);
        Assertions.assertEquals("A", patient.getName());
    }
}
